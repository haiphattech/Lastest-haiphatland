...
@section('content')
    <div class="content-wrapper">
        <div class="page-header">
            <h3 class="page-title">Tuyển dụng</h3>
                    ...
                    <div class="card-body">
                        <div>
                            <h4 class="card-title">
                                ...
                                <a href="{{route('recruits.create')}}" class="btn btn-primary btn-fw float-end">Thêm mới</a>
                                @endcan
                            </h4>
                            ...
                            <table class="table">
                                ...
                                @foreach($recruits as $item)
                                    <tr role="row">
                                        ...
                                        <td class="text-center">
                                            @can('update', $item)
                                                <a href="{{route('recruits.edit', $item['id'])}}" class="btn btn-primary btn-icon-text"><i class="mdi mdi-file-check btn-icon-prepend icon-mr"></i> Sửa</a>
                                            @endcan
                                            @can('delete', $item)
                                                <form class="d-inline-block" action="{{ route('recruits.destroy', $item['id']) }}" method="POST" >
                                                @csrf
                                                @method('DELETE')
